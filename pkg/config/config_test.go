...
func TestGetConfig(t *testing.T) {
	...
	cfg, err := GetConfig("../../test/data/configs/valid/accounts.yaml")
	require.Nil(err)
	...
	cfg, err = GetConfig("../../test/data/configs/valid/")
	require.Nil(err)
	assert.Equal("imap.server.de", cfg.Accounts["test"].Connection.Server)

	// Failed file/dir loading
	cfg, err = GetConfig("../../test/data/configs/does-not-exist")
	assert.EqualError(err, "stat ../../test/data/configs/does-not-exist: no such file or directory")

	cfg, err = GetConfig("../../test/data/configs/invalid/zero-file.yaml")
	...
	err = os.Chmod("../../test/data/configs/invalid-unreadable-file/unreadable-file.testfile.yaml", 0000)
	require.Nil(err)
	cfg, err = GetConfig("../../test/data/configs/invalid-unreadable-file/")
